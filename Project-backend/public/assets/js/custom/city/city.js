...
$(document).ready(function () {
    dtable = $('#list').DataTable({
        ...
        'columns':
            [
                ...
                {
                    "title":"Action", 'data': { "status": "status", "id": "_id" }, "orderable": false,
                    render: function (data) {
    ...
    $('body').on('click','.change_status',function(e){
        ...
        $.ajax({
            type: 'POST',
            url: AUTH_HTTP_PATH + 'city/status/change',
            dataType: 'json',
            data: {
                status: status,
                id: id
            ...
            success: function (data) {
                if (data.status == 1) {
                    show_toaster("City", data.message, "success");
                    dtable.ajax.reload();
                } else {
                    show_toaster("Oops!", data.message, "error");
                }
...
$('body').on('submit', '#edit', function (e) {
    ...
    $('#submit_btn').attr('disabled', true);
    $.ajax({
        url: AUTH_HTTP_PATH + "city/update",
        dataType: 'json',
        data: formData,
        type: 'POST',
        success: function (data) {
